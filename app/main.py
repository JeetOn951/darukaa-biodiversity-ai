...
with st.sidebar:
    ...
    soc = st.number_input("Soil Organic Carbon (%)", min_value=0.0, value=0.3, step=0.1)
    moisture = st.number_input("Soil Moisture (%)", min_value=0.0, max_value=100.0, value=20.0, step=1.0)
    rainfall = st.number_input("Annual Rainfall (mm)", min_value=0.0, value=500.0, step=50.0)
...
if st.button("Analyze", type="primary", use_container_width=True):
    try:
        ...
        with st.spinner("Retrieving knowledge and generating recommendations..."):
        ...
        st.write(", ".join(result["metrics"]) or "Additional measurements required")
